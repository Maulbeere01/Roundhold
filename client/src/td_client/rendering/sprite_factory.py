...
class SpriteFactory:
    ...
    def create_unit_sprite(self, entity_id: int, unit_type: str, player_id: str, x: float, y: float) -> UnitSprite:
        frames: List[pygame.Surface] = self.template_manager.get_unit_template(unit_type, player_id)
        sprite = UnitSprite(x=x, y=y, frames=frames, entity_id=entity_id)
        if self.units_group is not None:
            self.units_group.add(sprite)
        self.unit_sprites[entity_id] = sprite
        self.animation_manager.register(sprite)
        return sprite

    def create_tower_sprite(self, entity_id: int, tower_type: str, x: float, y: float, range_px: float) -> BuildingSprite | None:
        image = self.template_manager.get_tower_template(tower_type)
        if not image:
            return None
        sprite = BuildingSprite(x=x, y=y, image=image, entity_id=entity_id, range_px=range_px)
        if self.towers_group is not None:
