...
from .assets import AssetLoader, TemplateManager
...
class GameSimulation:
    ...
    def __init__(
        ...
        self._initialize_placement_grids()
    ...
    def _initialize_renderer(self) -> None:
        self.render_manager = RenderManager(
            ...
            TemplateManager(self.asset_loader),
            self.settings,
            TILE_SIZE_PX,
        ...
        self.render_manager.initialize(self)
    ...
    def _initialize_controllers(self) -> None:
        ...
        self.input_controller = InputController(
            self.build_controller, self.debug_renderer
        )
    ...
    def _initialize_ui_elements(self) -> None:
        ...
        if self.player_id == "A":
            ...
            tower_button_x = self.display_manager.screen_width - tower_button_w - 20
        tower_button_y = self.display_manager.screen_height - tower_button_h - 20
        self.tower_button = pygame.Rect(
        ...
        self._local_towers: dict[
            tuple[str, int, int], BuildingSprite
        ] = {}  # (player_id,row,col) -> sprite

    def get_player_grid(self, player_id: PlayerID) -> PlacementGrid:
        """Get the placement grid for a specific player."""
    ...
    def _initialize_placement_grids(self) -> None:
        """Create placement grids for both players on the global map."""
        self.placement_grid_A = PlacementGrid(self.map_cols, self.map_rows)
        self.placement_grid_B = PlacementGrid(self.map_cols, self.map_rows)
        self.placement_grid_A.populate_from_paths(GAME_PATHS["A"])
        self.placement_grid_B.populate_from_paths(GAME_PATHS["B"])
    ...
    def update(self, dt: float):
        ...
        if state:
            ...
            if not self._round_result_received:
                if self.player_id == "A":
                    my_vis = self._round_base_my_lives - int(
                        getattr(state, "lives_lost_player_A", 0)
                    )
                    opp_vis = self._round_base_opponent_lives - int(
        ...
        if self.in_combat:
            ...
            if (
                ...
                self.network_client.round_ack(
                    player_id=self.player_id, round_number=self.current_round
                )
                self._round_ack_sent = True

    def render(self) -> None:
        """Render game world, sprites, and UI."""
        self.render_manager.draw(self.terrain_map)
    ...
    def _on_build_response(
        ...
        *,
        ...
        if success:
            ...
            logger.warning(
                "BuildTower rejected by server at (%d,%d), rolling back", row, col
            )
            ...
            # Only remove sprite if we created it in this build attempt
            if not sprite_existed:
                # We created a new sprite, so remove it
                self._remove_local_tower(self.player_id, row, col)
                logger.info(
                    "Rolled back tower at (%d,%d) - removed sprite we created", row, col
                )
            else:
                # Sprite already existed (from previous successful build), don't remove it
                logger.info(
                    "Rolled back tower at (%d,%d) - sprite already existed, keeping it",
                    row,
                    col,
                )

            # Only clear grid if field was empty before our build attempt
            if was_empty:
                my_grid.clear_tower(row, col)
                logger.info("Rolled back tower at (%d,%d) - field cleared", row, col)
            else:
                logger.info(
                    "Rolled back tower at (%d,%d) - field was already occupied, not clearing",
                    row,
                    col,
                )
    ...
    def _on_send_units_response(self, success: bool, total_gold: int | None) -> None:
        ...
        if success and total_gold is not None:
            logger.info(
                "SendUnits acknowledged by server, updating gold to %d", total_gold
            )
...
class GameApp:
    ...
    def _on_round_result(self, round_result_pb) -> None:
        ...
        if self.game is None or self.player_id is None:
            return
        logger.info(
            "RoundResult: A -%d +%d, B -%d +%d (authoritative totals: A %d lives/%d gold, B %d lives/%d gold)",
