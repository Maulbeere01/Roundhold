...
# Hier definieren wir die Beschreibungen für die Tooltips
UNIT_DESCRIPTIONS = {
    "standard": [
        "Type: Warrior",
        "Role: Swarm / DPS",
...
class HUDRenderer:
    ...
    def render(self, game) -> None:
        ...
        tower_btn_color = (120, 200, 120) if game.ui_state.tower_build_mode else (80, 120, 80)
        pygame.draw.rect(surface, tower_btn_color, game.ui_state.tower_button, border_radius=8)
        tower_label = font.render("Tower", True, (255, 255, 255))
        tower_label_rect = tower_label.get_rect(center=game.ui_state.tower_button.center)
        ...
        if game.phase_state.in_preparation:
            prep_text = timer_font.render(
                f"Prep: {int(game.phase_state.prep_seconds_remaining)}s", True, (200, 220, 255)
            )
            surface.blit(prep_text, (center_x - prep_text.get_width() // 2, 20))

        if game.phase_state.in_combat:
            combat_text = timer_font.render(
                f"Combat: {int(game.phase_state.combat_seconds_remaining)}s", True, (255, 220, 200)
            )
        ...
        game.sim_state.debug_renderer.draw_info_text(game.display_manager.render_surface)

        # 8. Unit Selection & Tooltips
        tooltip_unit_type = None  # Speichern, ob wir etwas hovern
        ...
        if game.ui_state.unit_selection_buttons:
            for rect, u_type in game.ui_state.unit_selection_buttons:
                ...
                # Wenn gehovert, merken wir uns den Typ für später
                if is_hovered:
        ...
        # 9. Tooltip zeichnen (GANZ AM ENDE, damit er oben liegt)
    ...
    def _draw_unit_tooltip(self, surface: pygame.Surface, mx: int, my: int, unit_type: str) -> None:
        """Draws a tooltip box with unit stats."""
        ...
        # Farben
        bg_color = (20, 20, 30, 230) # Halbtransparenter dunkler Hintergrund
        border_color = (200, 200, 200)
        text_color = (220, 220, 220)
        stat_color = (100, 255, 100) # Grün für Stats

        # Inhalt vorbereiten
        lines = []
        
        # Stats Zeile (HP, Speed)
        hp = stats.get("health", "?")
        ...
        # Position (Rechts oberhalb der Maus)
        box_x = mx + 15
        box_y = my - box_height - 10
        
        # Verhindern, dass es aus dem Bildschirm ragt
        ...
        # Box Hintergrund (Surface für Alpha nötig)
        box_surf = pygame.Surface((box_width, box_height), pygame.SRCALPHA)
        pygame.draw.rect(box_surf, bg_color, box_surf.get_rect(), border_radius=5)
        pygame.draw.rect(box_surf, border_color, box_surf.get_rect(), width=1, border_radius=5)
