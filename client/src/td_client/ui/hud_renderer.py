...
class HUDRenderer:
    ...
    def render(self, game) -> None:
        ...
        my_hud = font.render(
            f"Gold: {game.my_gold}   Lives: {game.my_lives}", True, (255, 255, 255)
        )
        opp_hud = font.render(
            f"Opponent Lives: {game.opponent_lives}", True, (255, 200, 200)
        )
        if game.player_id == "B":
        ...
        if game.in_preparation:
            prep_text = timer_font.render(
                f"Prep: {int(game.prep_seconds_remaining)}s", True, (200, 220, 255)
            )
            surface.blit(prep_text, (center_x - prep_text.get_width() // 2, 20))
        if game.in_combat:
            combat_text = timer_font.render(
                f"Combat: {int(game.combat_seconds_remaining)}s", True, (255, 220, 200)
            )
        ...
        game.debug_renderer.draw_grid(
            ...
            placement_grid_left=game.placement_grid_A,
            placement_grid_right=game.placement_grid_B,
            left_origin=(game.terrain_map.rect.x, game.terrain_map.rect.y),
            right_origin=(game.terrain_map.rect.x, game.terrain_map.rect.y),
        )
