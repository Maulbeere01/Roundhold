...
import grpc
from td_shared.game import ROUND_ACK_TIMEOUT
from td_shared.game import SimulationData as SimDataDict
...
class GameRpcServer(game_pb2_grpc.YourGameServiceServicer):
    ...
    def __init__(
        self, game_manager: GameStateManager, round_manager: RoundManager
    ) -> None:
        ...
        self._waiting_clients: list[
            tuple[str, threading.Event, list[game_pb2.MatchEvent]]
        ] = []
    ...
    def _ensure_round_loop(self) -> None:
        if not self._round_thread_started:
            threading.Thread(
                target=self.round_manager.run_game_loop, daemon=True
            ).start()
            self._round_thread_started = True
            self._log.info("RoundManager thread started on first match")

    def BuildTower(
        self, request: game_pb2.BuildTowerRequest, context: grpc.ServicerContext
    ) -> game_pb2.BuildTowerResponse:  # type: ignore[override]
        """Place a tower."""
        ...
        if success:
            ...
            for ev, outbox in getattr(self.round_manager, "_active_clients", []):
                ...
                ev.set()
            self._log.info(
                "Broadcasted TowerPlaced for player=%s at (%d,%d)",
                request.player_id,
                request.tile_row,
                request.tile_col,
            )
    ...
    def SendUnits(
        self, request: game_pb2.SendUnitsRequest, context: grpc.ServicerContext
    ) -> game_pb2.SendUnitsResponse:  # type: ignore[override]
        ...
        try:
            if not self.round_manager.is_in_preparation():
                ...
                return game_pb2.SendUnitsResponse(
                    success=False, total_gold=current_gold
                )
            units = [
                {
            ...
            if ok:
                self._log.info(
                    "SendUnits accepted: player=%s, count=%d",
                    request.player_id,
                    len(units),
                )
                ...
                self._log.info(
                    "SendUnits rejected (gold insuff.): player=%s", request.player_id
                )
                return game_pb2.SendUnitsResponse(
                    success=False, total_gold=current_gold
                )
    ...
    def QueueForMatch(
        self, request: game_pb2.QueueRequest, context: grpc.ServicerContext
    ) -> game_pb2.MatchEvent:
        ...
        with self._match_lock:
            ...
            if len(self._waiting_clients) >= 2:
                ...
                sim_data_dict: SimDataDict = (
                    self.game_manager.get_current_state_snapshot()
                )
                ...
                o1.append(
                    game_pb2.MatchEvent(
                        match_found=game_pb2.MatchFound(
                            ...
                            initial_round_start=round_start,
                        )
                    )
                )
        ...
        try:
            ...
            while not ready.is_set():
                ...
                self._log.debug(
                    "QueueForMatch: sending queue_update to %s: %s", player_name, msg
                )

                yield game_pb2.MatchEvent(
                    queue_update=game_pb2.QueueUpdate(message=msg)
                )
            ...
            while context.is_active():
                ...
                while outbox:
                    ...
                    self._log.debug(
                        "QueueForMatch: delivering %s to %s", which, player_name
                    )
                    yield ev
    ...
    def RoundAck(
    ...
    ) -> game_pb2.RoundAckResponse:  # type: ignore[override]
        ...
        with self._round_ack_lock:
            ...
            self._log.info(
                "RoundAck: player=%s, round=%d, total_acks=%d",
                player_id,
                round_num,
                len(self._round_acks[round_num]),
            )
        return game_pb2.RoundAckResponse(success=True)

    def wait_for_round_acks(
        self, round_num: int, timeout: float = ROUND_ACK_TIMEOUT
    ) -> bool:
        ...
        while time.time() - start < timeout:
            with self._round_ack_lock:
                ...
                if "A" in acks and "B" in acks:
                    self._log.info(
                        "RoundAck: round %d complete (both players acked)", round_num
                    )
                    return True
