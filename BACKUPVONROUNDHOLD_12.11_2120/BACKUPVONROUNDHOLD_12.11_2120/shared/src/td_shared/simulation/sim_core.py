...
from ..game.protocol import PlayerID, SimulationData, SimTowerData, SimUnitData
from ..map.path_utils import mirror_paths_for_width
...
class SimUnit(SimEntity):
    ...
    def __init__(
        ...
        if route not in GAME_PATHS:
            raise ValueError(f"Unknown route: {route}")
        
        if player_id == PLAYER_B:
            mirrored_paths = mirror_paths_for_width({route: GAME_PATHS[route]}, MAP_WIDTH_TILES)
            path_tiles = mirrored_paths[route]
        else:
            path_tiles = GAME_PATHS[route]
    ...
    def take_damage(self, damage: int, attacker_player_id: PlayerID, game_state: "GameState") -> None:
...
class SimTower(SimEntity):
    ...
    def _find_target(self, enemy_units: List[SimUnit]) -> Optional[SimUnit]:
...
class GameState:
    ...
    def __init__(self, simulation_data: SimulationData):
        ...
        self.last_unit_inactive_tick: Optional[int] = None
        ...
        for tower_id, tower_data in enumerate(simulation_data["towers"]):
            # Mirror tower positions for player B to match mirrored unit paths
            tower_x = tower_data["position_x"]
            if tower_data["player_id"] == PLAYER_B:
                tower_x = map_width_px - tower_x
        ...
        self._unit_spawn_ticks = {
            ...
            for unit, unit_data in zip(self.units, simulation_data["units"])
        }
