...
class FoamRenderer(AnimatedObject):
    ...
    def __init__(
        ...
        Args:
            ...
            foam_positions: List of (x, y, is_left_map) pixel positions where foam should appear
    ...
    def _scale_frames(self, frames: List[pygame.Surface]) -> List[pygame.Surface]:
        ...
        scaled_height = int(original_frame_height * base_scale_factor * self.FOAM_SCALE_FACTOR)
    ...
    def _calculate_horizontal_inset(
        self, idx: int, total_count: int
    ) -> int:
    ...
    def _render_foam_at_position(
        ...
        if foam_width > self.tile_size:
            draw_x = (
                x + (self.tile_size - foam_width) // 2 + horizontal_inset
            )
        else:
    ...
    def get_surface(self) -> pygame.Surface:
        ...
        right_positions = sorted(
            [
                (x, y)
                for x, y, is_left in self.foam_positions
                if not is_left
            ],
            key=lambda p: p[0]
        )
